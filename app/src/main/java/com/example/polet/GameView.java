...
import com.example.polet.entity.Asteroid;
import com.example.polet.entity.Background;
import com.example.polet.entity.Bullet;
import com.example.polet.entity.Flight;
...
public class GameView extends SurfaceView implements Runnable{
   ...
   //     // задание скорости подъёма и снижения самолёта
        ...
        if (flight.getY() < -flight.getHeight()/2) { // запрет на снижение меньше нуля
            flight.setY(flight.getHeight()*3/4);
        } else if (flight.getY() >=screenY - flight.getHeight()*3/4) { // запрет на подъём выше экрана за минусом высоты самолёта
            flight.setY(flight.getHeight()*3/4);
        }
